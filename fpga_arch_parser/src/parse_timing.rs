...
pub fn parse_delay_constant(
    ...
    let (min, max) = match min {
        ...
        None => match max {
            ...
            None => {
                return Err(FPGAArchParseError::MissingRequiredAttribute(
                    "At least one of the max or min attributes must be specified".to_string(),
                    parser.position(),
                ));
            }
        },
    ...
    for line in text.split('\n') {
        ...
        for v in line.split_whitespace() {
            let element_val: f32 = match v.parse() {
                ...
                Err(e) => {
                    return Err(FPGAArchParseError::InvalidTag(
                        format!("Matrix text parse error: {e}"),
                        parser.position(),
                    ));
                }
            };
...
pub fn parse_delay_matrix(
            ...
            "type" => {
                delay_type = match delay_type {
                    None => match a.value.as_ref() {
                        ...
                        _ => {
                            return Err(FPGAArchParseError::AttributeParseError(
                                format!("Unknown delay type: {}", a.value),
                                parser.position(),
                            ));
                        }
                    },
        ...
        match parser.next() {
            Ok(XmlEvent::Characters(text)) => {
                matrix = match matrix {
                    ...
                    Some(_) => {
                        return Err(FPGAArchParseError::InvalidTag(
                            "Duplicate characters within delay_matrix.".to_string(),
                            parser.position(),
                        ));
                    }
                }
    ...
    let matrix = match matrix {
        ...
        None => {
            return Err(FPGAArchParseError::InvalidTag(
                "delay_matrix tag must contain text representation of the matrix data".to_string(),
                parser.position(),
            ));
        }
    };
