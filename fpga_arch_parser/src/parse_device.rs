...
fn parse_device_area(
    ...
    let grid_logic_tile_area = match grid_logic_tile_area {
        ...
        None => {
            return Err(FPGAArchParseError::MissingRequiredAttribute(
                "grid_logic_tile_area".to_string(),
                parser.position(),
            ));
        }
    };
            ...
            "type" => {
                sb_type = match sb_type {
                    None => match a.value.as_ref() {
                        ...
                        _ => {
                            return Err(FPGAArchParseError::AttributeParseError(
                                format!("Unknown switch_block type: {}", a.value),
                                parser.position(),
                            ));
                        }
                    },
...
fn parse_chan_w_dist(
    ...
    match distr.as_str() {
        ...
        "uniform" => Ok(ChanWDist::Uniform(UniformChanWDist { peak })),
        "pulse" => {
            let width = match width {
                Some(n) => n,
                None => {
        ...
        "delta" => {
            ...
            Ok(ChanWDist::Delta(DeltaChanWDist { peak, xpeak, dc }))
        }
        _ => Err(FPGAArchParseError::AttributeParseError(
            format!("Unknown distr for chan_w_distr: {distr}"),
            parser.position(),
        )),
    }
}
                ...
                match name.to_string().as_str() {
                    ...
                    "connection_block" => {
                        connection_block = match connection_block {
                            None => {
                                Some(parse_device_connection_block(&name, &attributes, parser)?)
                            }
                            Some(_) => {
                                return Err(FPGAArchParseError::DuplicateTag(
                                    name.to_string(),
