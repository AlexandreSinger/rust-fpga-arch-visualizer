...
pub fn parse_crr_lane_num(lane_num_str: &str) -> Result<usize, CRRSBParseError> {
    match lane_num_str.parse() {
        Ok(v) => Ok(v),
        Err(e) => Err(CRRSBParseError::SBHeaderCellParseError(format!(
            "Invalid lane num string: {e}."
        ))),
    }
}
