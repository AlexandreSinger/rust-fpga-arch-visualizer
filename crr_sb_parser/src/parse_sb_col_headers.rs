...
use crate::{CRRSBParseError, crr_sb_des::CRRSwitchColHeaderInfo, parse_common::parse_crr_switch_dir};

pub fn parse_col_headers(csv_records: &mut StringRecordsIter<'_, File>) -> Result<Vec<CRRSwitchColHeaderInfo>, CRRSBParseError> {
    let mut col_headers: Vec<CRRSwitchColHeaderInfo> = Vec::new();
    ...
    for i in 4..num_cols {
        col_headers.push(CRRSwitchColHeaderInfo {
            ...
            lane_num: match lane_num_row[i].trim().parse() {
                Ok(v) => v,
                Err(e) => {
                    return Err(CRRSBParseError::SBHeaderCellParseError(
                        format!("{e}")
