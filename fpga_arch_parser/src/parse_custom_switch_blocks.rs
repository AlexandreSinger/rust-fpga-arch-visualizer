...
fn parse_switchblock_location(name: &OwnedName,
                              attributes: &[OwnedAttribute],
                              parser: &mut EventReader<BufReader<File>>) -> Result<CustomSwitchBlockLocation, FPGAArchParseError> {
            ...
            "type" => {
                switchblock_location = match switchblock_location {
                    None => match a.value.to_string().as_ref() {
                        ...
                        "XY_SPECIFIED" => Some(CustomSwitchBlockLocation::XYSpecified { x: -1, y: -1 }),
                        _ => return Err(FPGAArchParseError::AttributeParseError(format!("{a}: Unknown custom switch block location"), parser.position())),
                    },
    ...
    let switchblock_location = match switchblock_location {
        CustomSwitchBlockLocation::XYSpecified { .. } => {
            match x {
                Some(x) => match y {
        ...
        _ => {
            ...
            if x.is_some() {
                return Err(FPGAArchParseError::UnknownAttribute("x".to_string(), parser.position()));
            }
            if y.is_some() {
                return Err(FPGAArchParseError::UnknownAttribute("y".to_string(), parser.position()));
            }
...
fn parse_func(name: &OwnedName,
              attributes: &[OwnedAttribute],
              parser: &mut EventReader<BufReader<File>>) -> Result<CustomSwitchFunc, FPGAArchParseError> {
    ...
    let formula = match formula {
        Some(p) => p,
        None => return Err(FPGAArchParseError::MissingRequiredAttribute("formula".to_string(), parser.position())),
    };
...
fn parse_switchfuncs(name: &OwnedName,
                     attributes: &[OwnedAttribute],
                     parser: &mut EventReader<BufReader<File>>) -> Result<Vec<CustomSwitchFunc>, FPGAArchParseError> {
...
fn parse_switchpoint_list(switchpoint_list: &str,
                          parser: &mut EventReader<BufReader<File>>) -> Result<Vec<i32>, FPGAArchParseError> {
    let mut switchpoints: Vec<i32> = Vec::new();

    for substr in switchpoint_list.split(',') {
        let switchpoint = match substr.parse() {
            Ok(v) => v,
            Err(e) => return Err(FPGAArchParseError::AttributeParseError(format!("switchpoint parse error: {e}"), parser.position())),
        };
...
fn parse_conn_point(name: &OwnedName,
                    attributes: &[OwnedAttribute],
                    parser: &mut EventReader<BufReader<File>>) -> Result<CustomSwitchBlockConnPoint, FPGAArchParseError> {
    ...
    let switchpoint = match switchpoint {
        Some(p) => p,
        None => return Err(FPGAArchParseError::MissingRequiredAttribute("switchpoint".to_string(), parser.position())),
    };
...
fn parse_wireconn(name: &OwnedName,
                  attributes: &[OwnedAttribute],
                  parser: &mut EventReader<BufReader<File>>) -> Result<CustomSwitchWireConn, FPGAArchParseError> {
            ...
            "from_order" => {
                from_order = match from_order {
                    None => match a.value.to_string().as_ref() {
                        ...
                        _ => return Err(FPGAArchParseError::AttributeParseError(format!("{a}: unknown custom switch block connection order."), parser.position())),
                    },
                    Some(_) => return Err(FPGAArchParseError::DuplicateAttribute(a.to_string(), parser.position())),
                }
            },
    ...
    let num_conns = match num_conns {
        Some(p) => p,
        None => return Err(FPGAArchParseError::MissingRequiredAttribute("num_conns".to_string(), parser.position())),
    };
    ...
    let mut from_points = match from_type {
        Some(from_types) => match from_switchpoint {
            ...
            None => return Err(FPGAArchParseError::MissingRequiredAttribute("from_switchpoint".to_string(), parser.position())),
        },
        None => match from_switchpoint {
            Some(_) => return Err(FPGAArchParseError::MissingRequiredAttribute("from_type".to_string(), parser.position())),
            None => Vec::new(),
        },
    };
    let mut to_points = match to_type {
        Some(to_types) => match to_switchpoint {
            ...
            None => return Err(FPGAArchParseError::MissingRequiredAttribute("to_switchpoint".to_string(), parser.position())),
        },
        None => match to_switchpoint {
            Some(_) => return Err(FPGAArchParseError::MissingRequiredAttribute("to_type".to_string(), parser.position())),
            None => Vec::new(),
        },
    };
    ...
    if from_points.is_empty() {
        return Err(FPGAArchParseError::InvalidTag("from_points is empty".to_string(), parser.position()));
    }
    if to_points.is_empty() {
        return Err(FPGAArchParseError::InvalidTag("to_points is empty".to_string(), parser.position()));
    }
...
fn parse_switchblock(name: &OwnedName,
                     attributes: &[OwnedAttribute],
                     parser: &mut EventReader<BufReader<File>>) -> Result<CustomSwitchBlock, FPGAArchParseError> {
            ...
            "type" => {
                sb_type = match sb_type {
                    None => match a.value.to_string().as_ref() {
                        ...
                        _ => return Err(FPGAArchParseError::AttributeParseError(format!("{a}: unknown custom switch block type"), parser.position())),
                    },
                    Some(_) => return Err(FPGAArchParseError::DuplicateAttribute(a.to_string(), parser.position())),
                }
            },
    ...
    let switchblock_location = match switchblock_location {
        Some(t) => t,
        None => return Err(FPGAArchParseError::MissingRequiredTag("<switchblock_location>".to_string())),
    };
    // FIXME: The documentation is not clear if this is required.
    let switch_funcs = match switch_funcs {
        Some(t) => t,
        None => return Err(FPGAArchParseError::MissingRequiredTag("<switchfuncs>".to_string())),
    };
...
pub fn parse_switchblocklist(name: &OwnedName,
                             attributes: &[OwnedAttribute],
                             parser: &mut EventReader<BufReader<File>>) -> Result<Vec<CustomSwitchBlock>, FPGAArchParseError> {
